...
const getNetworkType = (chainId: number) => CHAINS[chainId].isTestnet ? 'testnet' : 'mainnet'

const getNetworkAgnosticServiceUrlPrefix = (chainId: number, isDev: boolean) => `${getDevPrefix(
    isDev,
...
const getIdentityAPIUrl = (chainId: number, isDev = false) => `https://${getNetworkAgnosticServiceUrlPrefix(chainId, isDev)}.identity-verification.huma.finance`

const getAuthServiceUrl = (chainId: number, isDev = false) => `https://${getNetworkAgnosticServiceUrlPrefix(chainId, isDev)}.auth.huma.finance`
